...
export function ChatInput({
  ...
  return (
    <div className="p-4 border-t border-gray-200 dark:border-border bg-gray-50 dark:bg-card/80 shrink-0">
      <div className="flex gap-2 items-end">
        ...
        <div className="relative group">
          <Button
            ...
            disabled={disabled}
            className={cn(
              "shrink-0 h-10 w-10 rounded-xl transition-colors border-gray-300 dark:border-border",
              showQuickActions
        ...
        <textarea
          ...
          rows={1}
          className="flex-1 min-h-10 max-h-[120px] px-4 py-2.5 bg-white dark:bg-background border border-gray-200 dark:border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent resize-none text-sm text-foreground placeholder:text-gray-400 outline-none scrollbar-hide"
        />
      ...
      <p className="text-[10px] text-gray-400 dark:text-gray-500 text-center mt-2">
        ...
        for new line • Powered by {tenant?.displayName ?? "Lowfield Pharmacy"}
      </p>
    </div>
  );
}
